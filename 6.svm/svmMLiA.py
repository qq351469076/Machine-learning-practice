...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    while iter < maxIter:  # 循环次数 小于 取消前循环的最大次数
        alphaPairsChanged = 0  # 记录alpha是否已经进行过优化
        for i in range(m):  # 遍历所有样本
            ...
            print('')
            print('预测的类别为: ' + str(fXi))
            Ei = fXi - float(labelMat[i])  # 如果检查一个例子是否违反KKT条件
            print('误差为: ' + str(Ei))
            """
                如果误差很大, 对该数据实例所对应的alpha进行优化, 不管是正间隔还是负间隔
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                print('优化后的预测类别为: ' + str(fXj))
                Ej = fXj - float(labelMat[j])  # 计算误差
                print('优化后的误差: ' + str(Ej))
                alphaIold = alphas[i].copy()  # 拷贝一份当前样本的alpha I
                alphaJold = alphas[j].copy()  # 拷贝一份随机样本的alpha J
                # 保证alpha J在0和C之间, 已经在边界上了, 不能够增加或减少了, 因此不能在继续优化了
                if labelMat[i] != labelMat[j]:  # 当两个样本不一样的时候
                    print('两个样本当前一样')
                ...
                """
                    eta是alpha[J]的最优修改量
                    ((2.0 * 当前样本) * 随机当前样本.T) - (当前样本 * 当前样本.T) - (随机当前样本 * 随机当前样本.T)
                    (1, 1) - (1, 1) - (1, 1)
                """
                ...
                if np.abs(alphas[j] - alphaJold) < 0.00001:
                    print "j 不够移动"
                    continue
                alphas[i] += labelMat[j] * labelMat[i] * (alphaJold - alphas[j])  # 按J的相同数量更新I, 但方向相反
...
if __name__ == '__main__':
    ...
    # print(dataset)
    # print(label)
    smoSimple(dataset, label, 0.6, 0.001, 40)
